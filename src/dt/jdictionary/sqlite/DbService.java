...
public class DbService 
{
	...
	{
		final List<RawDbRow> rawResults = db.lookupChinese(zh);
		final Map<String, List<String>> resultsByPinyin = new HashMap<>();
		for(final RawDbRow rawResult : rawResults)
		{
	...
	{
		final List<RawDbRow> rawResults = db.lookupEnglish(en);
		return convertRawToSimple(rawResults);
	}

	private List<SimpleLookup> convertRawToSimple(List<RawDbRow> rawResults)
	{
